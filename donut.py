...
label = tk.Label(window, text=render_frame(1,1), font=("Courier", 10), fg="green", bg="black")
label.pack()
...
def update_text():
    ...
    A += 0.08
    B += 0.03
    new_text = render_frame(A, B)
    
    label.config(text=str(new_text))
    label.after(30, update_text)  # Update the text every 0.03 second
